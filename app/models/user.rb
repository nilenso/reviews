class User < ActiveRecord::Base
  ...
  has_settings do |s|
    s.key :theme, :defaults => { :color_scheme => "dark" }
  end
  ...
  def stdev_suggested_level
    suggested_levels = self.reviews_for.done.map(&:suggested_level).compact
    suggested_levels.stdev if suggested_levels.present? && suggested_levels.count > 1
  end
